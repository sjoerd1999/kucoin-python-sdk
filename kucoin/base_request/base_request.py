...
import json
#import requests
import kucoin.base_request.requests_pycurl as requests      # Swap for normal requests library
import hmac
import hashlib
import base64
import time
...
session = requests.Session()  # MB05262021
...
class KucoinBaseRestApi(object):
    ...
    def _request(self, method, uri, timeout=5, auth=True, params=None):
        ...
        if method in ['GET', 'DELETE']:
            ...
            response_data = session.request(method, url, headers=headers, timeout=timeout)  # MB05262021
        else:
            # response_data = requests.request(method, url, headers=headers, data=data_json,
            #                                 timeout=timeout)
            response_data = session.request(method, url, headers=headers, data=data_json, timeout=timeout)  # MB05262021 
